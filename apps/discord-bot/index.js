...
// Log in to Discord when credentials are available.
// The API server can still run locally without bot credentials.
if (process.env.DISCORD_TOKEN) {
  client.login(process.env.DISCORD_TOKEN).catch((err) => {
    console.error("Failed to log in to Discord:", err);
  });
} else {
  console.warn("DISCORD_TOKEN is not set; Discord bot login skipped.");
}
